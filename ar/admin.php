<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Admin Dashboard Lines
    |--------------------------------------------------------------------------
    |
    |
    */
    'dashboard' => [
        'title' => 'لوحة التحكم',
        'total-members' => 'إجمالي الأعضاء',
        'claimed-profiles' => 'الحسابات الخاصه بك',
        'profiles' => 'الحسابات',
        'skins' => 'السكنات',
        'users' => 'المستخدمين',
        'last-30-days' => 'آخر 30 يوماً',
        'members' => 'الأعضاء',
        'usernames' => 'اسماء المستخدمون',
    ],
    'help' => [
        'title' => 'المساعدة',
        'subtitle' => 'نصائح مفيدة للوحة تحكم SkinMC',
    ],
    'users' => [
        'title' => 'المستخدمين',
        'subtitle' => ':count مجموع المستخدمين المسجلين',
        'list' => [
            'username' => 'إسم المستخدم',
            'email' => 'البريد الإلكتروني',
            'status' => 'الحالة',
            'role' => 'الرتبه'
        ],
    ],
    'skins' => [
        'title' => 'السكنات',
    ],
    'tags' => [
        'title' => 'التصنيفات',
        'pending' => 'العلامات المعلقة',
        'banned' => 'العلامات المحظورة',
    ],
    'reports' => [
        'title' => 'الشكاوى',
    ],
    'feed' => [
        'title' => 'الاقتراحات',
        'subtitle' => ':count مجموع البوستات التي تم إنشاؤها',
    ],
    'wallpapers' => [
        'title' => 'الخلفية',
        'challenge' => [
            'title' => 'تحدي خلفية الشاشة',
            'description' => 'حدد الفائز لـ :challenge_الشهر. سيتم عرض الخلفية المختارة على الصفحة الرئيسية في :display_شهر.',
            'back_to_moderation' => 'العودة إلى الاعتدال',
            'month_label' => 'شهر التحدي',
            'previous_month' => 'الشهر الماضي',
            'next_month' => 'الشهر التالي',
            'shortlisted' => ':count قصير',
            'winner_selected' => 'الفائز المختار',
            'no_winner_selected' => 'الفائز المختار',
            'selected_winner' => 'Selected winner',
            'displays_on_home' => 'Displays on home in :month',
            'open_wallpaper' => 'Open wallpaper',
            'clear_reason_placeholder' => 'Reason for clearing (optional)',
            'clear_confirm' => 'Clear winner for :month?',
            'clear_winner' => 'Clear winner',
            'winner_unavailable' => 'The selected wallpaper is unavailable.',
            'display_month' => 'Display month: :month',
            'choose_from_shortlist' => 'Choose a wallpaper from the shortlist below.',
            'recent_changes' => 'Recent changes',
            'action_replaced' => 'Replaced winner',
            'action_set' => 'Set winner',
            'action_cleared' => 'Cleared winner',
            'log_actor' => 'by :name',
            'log_none' => 'None',
            'log_empty' => 'No winner changes logged for this month.',
            'table_rank' => '#',
            'table_wallpaper' => 'Wallpaper',
            'table_creator' => 'Creator',
            'table_stats' => 'Stats',
            'table_action' => 'Action',
            'stats_views' => ':count views',
            'stats_downloads' => ':count downloads',
            'stats_favourites' => ':count favourites',
            'reason_placeholder' => 'Reason (optional)',
            'set_winner' => 'Set winner',
            'selected' => 'Selected',
            'no_entries' => 'No eligible challenge entries for :month.',
            'unknown_creator' => 'Unknown creator',
            'unknown_user' => 'Unknown',
            'system_user' => 'System',
            'flash_saved' => 'Challenge winner saved.',
            'flash_cleared' => 'Challenge winner cleared.',
            'flash_none_to_clear' => 'No winner was set for this month.',
        ],
    ],
    'comments' => [
        'title' => 'التعليقات',
        'subtitle' => ':count مجموع التعليقات',
        'deleted' => [
            'title' => 'التعليقات المحذوفة',
            'subtitle' => ':count مجموع التعليقات المحذوفة',
        ]
    ]
];
